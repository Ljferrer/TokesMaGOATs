...
## Ephemeral Snipe reviews

Snipe's `codex exec --ephemeral` reviewers do not write normal session histories. Sync also imports retained Snipe coordinator results from temporary folders, repositories under ~/GitHub, Codex worktrees, and recorded Codex tool outputs. Repository discovery searches JSON filenames containing snipe and JSON files inside snipe directories, skipping dependency and Git internals. Configure `snipe_results` as a list of file paths or glob patterns in config.json for results saved elsewhere; use `snipe_search_roots` to configure recursively searched directories. Empty lists disable those file searches; configured Codex histories still supply recorded tool outputs. Only structured results containing reviewer thread IDs and completed-turn usage counters are counted. Copies deduplicate by thread/turn, and native session records supersede these aggregates. Imported counters survive deletion of temporary results.

Models use the recorded coordinator profile. CLI results lack per-response timestamps, so recovered usage is assigned to the reviewer thread's UUIDv7 start date; reviews spanning midnight can be attributed to the prior day. These aggregates count as one usage record per completed turn, not one per model response. Unsaved, deleted-before-import, incomplete or truncated results cannot supply missing counters unless a full copy survives in recorded tool output. Coverage explicitly flags retained summaries without usage counters; the warning applies globally because those stripped summaries may not retain reliable dates. No review text is stored in the ledger.
