...
assert.equal(buttons.at(-1).onmouseenter,undefined);assert.equal(buttons.at(-1).onfocus,undefined);
assert.equal(elements.get('dayPie').children[0].style.width,'220px');
buttons.at(-1).onclick();assert.equal(elements.get('dayPie').textContent,'No recorded tokens for this day.');
